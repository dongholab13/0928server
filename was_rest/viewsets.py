...
class MediaFileViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        if request.method =="POST":
            ...
            f = open( os.path.join(os.path.join(settings.MEDIA_ROOT, 'stream_1') , 'female.m3u8') , 'a')
            ##EXTINF:16.666667,
            #http://192.168.0.43:9031/file/female0.ts
            f.write("#EXTINF:5.0\n")
            f.write("http://192.168.0.43:9031/stream_1/" + ts_file_name + '\n')
            f.close()
...
class HeatmapViewset(viewsets.ModelViewSet):
    ...
    @action(detail=False)
    def returnHeatmap(self, requests, *args, **kwargs):
        return Response("testststs")



class SendImageViewset(viewsets.ModelViewSet):
...
class SendStreamTrajectoryViewset(viewsets.ModelViewSet):
    queryset = StreamTrajectory.objects.all()
    serializer_class = StreamTrajectorySerializer

    @action(methods=['POST'],detail=False)
    def singleImage(self, requests, *args, **kwargs):
        file_path = self.request.data['file_path']
        target_stream = Stream_trajectory.objects.get(pk=1)
        target_stream.file_list = str(target_stream.file_list) + file_path + ","
        target_stream.save()
        print(file_path)
        return Response("testststs")
    ...
    @action(methods=['GET'],detail=False)
    def getImage(self, requests, *args, **kwargs):
        target_stream = StreamTrajectory.objects.get(pk=1)
        file_path = target_stream.file_list.split(',')[0]
        target_stream.file_list = target_stream.file_list.replace(file_path + ',','')
        target_stream.save()
        if file_path == '':
